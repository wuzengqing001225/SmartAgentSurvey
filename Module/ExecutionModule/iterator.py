...
def questionnaire_iterator_segment(config_set, processed_data, question_segments, execution_order, sample_space, sample_space_size, sample_dimensions, upload = False, progress_file=None, multi_modal=False):
    ...
    for agent_id in range(sample_space_size):
        ...
        while current_question <= survey_size:
            ...
            format_part = """The output format should be in JSON, with each value structured as "question number: answer". Do not place ```json at the beginning. If you are asked to reason before/after answering a question, please put your reason and answer to the question in nested keys, like this: "question number: { 'reason': XXX, 'answer': XXX }". But if you are not asked to give a reason, just put the answer in the value of the question number key and do not give the reason. Please obey the rule strictly and do not use any other format."""
            if multi_modal:
